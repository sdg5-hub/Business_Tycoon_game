(() => {
  ...
  const el = {
    ...
    gameOverSaveBtn: document.getElementById("gameOverSaveBtn")
  };
  ...
  function updateView() {
    ...
    el.runDayBtn.disabled = state.gameOver;
    el.runDayBtn.textContent = state.gameOver ? "☠ BANKRUPT" : `▶ RUN DAY ${state.day}`;
  ...
  function runDay() {
    ...
    const operatingCost = 2.8 + state.stands * 1.6 + state.adLevel * 1.25 + state.sweetness * 0.9 + state.pressure * 0.85;

    state.stock = Math.max(0, state.stock - sold - waste);
    state.cash += revenue - operatingCost;
    state.reputation = Math.max(0, Math.min(20, state.reputation + repDeltaForPrice(state.price) + state.sweetness * 0.02));

    addLog(`Sold ${sold}/${demand} cups, wasted ${waste}, revenue ${toMoney(revenue)}, costs ${toMoney(operatingCost)}.`);
    ...
    if (state.cash <= 0) {
      state.cash = 0;
      state.gameOver = true;
      addLog("Business collapsed. You hit $0 and went bankrupt.");
      openGameOverModal("You went bankrupt after today's results.");
    }
  ...
  function newGame() {
    ...
    el.priceInput.value = state.price.toFixed(2);
    el.autoBuyToggle.checked = false;
    el.roomCodeInput.value = "";
    el.multiplayerStatus.textContent = "No room active.";
    closeGameOverModal();
  ...
  function openGameOverModal(reason) {
    ...
    el.gameOverSummary.textContent = `Final day: ${state.day} • Cash: ${toMoney(state.cash)} • Reputation: ${Math.round(state.reputation)} • Stands: ${state.stands} • Score: ${scoreValue()}`;
    el.gameOverModal.classList.add("open");
    el.gameOverModal.setAttribute("aria-hidden", "false");
  }

  function closeGameOverModal() {
    el.gameOverModal.classList.remove("open");
    el.gameOverModal.setAttribute("aria-hidden", "true");
  }
  ...
  function bind() {
    ...
    el.gameOverNewBtn.addEventListener("click", newGame);
    el.gameOverSaveBtn.addEventListener("click", () => {
      saveCurrentScore();
      addLog("Game-over score saved.");
      closeGameOverModal();
    });
  }
